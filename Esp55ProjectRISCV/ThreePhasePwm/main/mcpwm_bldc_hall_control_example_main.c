...
#define BLDC_MCPWM_TIMER_RESOLUTION_HZ 40000000 // 40MHz, 1 tick = 0.025us
#define BLDC_MCPWM_PERIOD              4000      // 100us, 10KHz
...
#define BLDC_PWM_VH_GPIO          8
#define BLDC_PWM_VL_GPIO          20
#define BLDC_PWM_WH_GPIO          23
#define BLDC_PWM_WL_GPIO          4
#define HALL_CAP_U_GPIO           3
...
static bldc_hall_phase_action_t s_hall_actions[] = {
...
static volatile uint8_t stop_cnt = 0;
static volatile uint8_t full_cnt = 0; 
static volatile uint8_t empty_cnt = 0; 

static bool full_callback(mcpwm_timer_handle_t timer, const mcpwm_timer_event_data_t *edata, void *user_ctx)
{
    full_cnt++;
    return false;
}

static bool empty_callback(mcpwm_timer_handle_t timer, const mcpwm_timer_event_data_t *edata, void *user_ctx)
{
    empty_cnt++;
    mcpwm_cmpr_handle_t *cmprs = (mcpwm_cmpr_handle_t *)user_ctx;
    uint32_t cur_speed_up = 600;
    for (int i = 0; i < 3; i++) {
        ESP_ERROR_CHECK(mcpwm_comparator_set_compare_value(cmprs[i], cur_speed_up));
        cur_speed_up += 500;
    }
    return false;    
}

static bool stop_callback(mcpwm_timer_handle_t timer, const mcpwm_timer_event_data_t *edata, void *user_ctx)
{
    stop_cnt++;
    return false;
}

static const mcpwm_timer_event_callbacks_t timer_cb_functions = 
{
    .on_full = NULL,
    .on_empty = &empty_callback,
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(mcpwm_timer_register_event_callbacks(timer, &timer_cb_functions, comparators));
...
//
    ...
    for (int i = 0; i < 3; i++) {
        ESP_ERROR_CHECK(mcpwm_generator_set_force_level(generators[i][BLDC_MCPWM_GEN_INDEX_HIGH], 1, false));
        // because gen_low is inverted by dead time module, so we need to set force level to 1
        ESP_ERROR_CHECK(mcpwm_generator_set_force_level(generators[i][BLDC_MCPWM_GEN_INDEX_LOW], 0, false));
    }
